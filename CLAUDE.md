...
The same local-copy pattern generates `src/calc_opd_mod.F90` from ExoRT's n68equiv source with one line patched: the k-coefficient pressure lookup is **clamped at the 10-bar table top** (`pressure = min(log10(pmid), log10pgrid(kc_npress))`). Above its pressure grid ExoRT linearly *extrapolates* k in log10(p), which is unphysical for >10 bar columns (outer-HZ dense-CO2 sweeps) and can produce negative k; the clamp freezes pressure broadening at its 10-bar value for deeper layers while CIA/continuum amagats keep the true `pmid`. Columns below 10 bar are bit-identical. The Makefile fails loudly (`$(error ...)`) if the sed anchor line disappears in a future ExoRT update. Both generated files are gitignored.
...
- **`exocol_config`** — Reads `exocol_config.nml`. Three optional namelist blocks:
  - **`&exocol_nml`** (runtime physics): `conv_scheme` ∈ {`'dry'`, `'moist'`, `'manabe'`, `'zm'`, `'sbm'`} (default `'sbm'`, the recommended simplified Betts-Miller scheme), `moisture_scheme` ∈ {`'prognostic'`, `'fixed_rh'`, `'off'`}, `o3_profile` ∈ {`'uniform'`, `'earth'`, `'none'`, `'rcemip'`} (default `'uniform'`; see `exocol_ozone`), `wind_speed` (default 5 m/s), `C_D` (default 1.5e-3; legacy `'bulk'` exchange coefficient), `msdist` (planet-star distance in AU, default 1.0; TOA stellar flux scales as 1/msdist²), `rh_sbm` (default 0.7; SBM reference relative humidity), `surface_flux` ∈ {`'mos'`, `'bulk'`} (default `'mos'`; see `exocol_surface`), `z0_rough` (default 3.21e-5 m; surface roughness used by `'mos'`), `latent_heat_mode` ∈ {`'phase_aware'`, `'fixed_vap'`}, `esat_formula` ∈ {`'cc'`, `'steam'`} (default `'cc'`; `'steam'` = Wagner & Pruß 2002 saturation pressure accurate to the 647 K critical point, applied model-wide via the `exocol_convadj::esat` dispatcher), `h2o_eos` ∈ {`'ideal'`, `'nonideal'`} (default `'ideal'`; `'nonideal'` = Kasting 1988 Appendix-A general non-ideal moist pseudoadiabat for the cold-start temperature profile — see `exocol_steam`/`exocol_iapws95` — and forces `esat_formula='steam'`), `co2_condense` and `cp_co2_tdep` (both default `.false.`; outer-HZ cold-start CO2 condensation + T-dependent cp_CO2 — see `exocol_co2`). `exocol_io::read_initial_conditions` copies `cfg_msdist` into `exocol_mod::msdist` after the input file is read (file mode); `exocol_coldstart::cold_start_init` does the equivalent for cold-start mode.
