...
Default is **70 layers**, **log-spaced** from `p_top` to `ps` (lowest level ~400 m). This is the recommended production grid: it gives Earth-like bulk surface fluxes with the standard `C_D` and a numerically well-behaved column. A hybrid grid with fine near-surface resolution (`n_sfc_layers > 0`, bottom midpoint ~8.5 m) is available but **reserved for future work that adds explicit boundary-layer mixing** — see `n_sfc_layers` in the `&exocol_nml` section. Override layer count via `config.mk` (`PVER = 80`) or on the command line:
...
- **`exocol_config`** — Reads `exocol_config.nml`. Three optional namelist blocks:
  - **`&exocol_nml`** (runtime physics): `conv_scheme` ∈ {`'dry'`, `'moist'`, `'manabe'`, `'zm'`, `'sbm'`} (default `'sbm'`, the recommended simplified Betts-Miller scheme), `moisture_scheme` ∈ {`'prognostic'`, `'fixed_rh'`, `'off'`}, `o3_profile` ∈ {`'uniform'`, `'earth'`, `'none'`} (default `'uniform'`; see `exocol_ozone`), `wind_speed` (default 5 m/s), `C_D` (default 1.5e-3), `msdist` (planet-star distance in AU, default 1.0; TOA stellar flux scales as 1/msdist²), `rh_sbm` (default 0.7; SBM reference relative humidity), `n_sfc_layers` (**default 0 = pure log-spacing, recommended**; lowest level ~400 m, Earth-like surface fluxes), `dp_sfc_bot` (default 200 Pa) and `sfc_stretch` (default 1.5) configure the optional hybrid near-surface grid when `n_sfc_layers > 0` (bottom midpoint ~8.5 m). The hybrid grid is reserved for future boundary-layer-mixing work: without BL mixing the thin bottom layer equilibrates to Ts (suppressing LE/SH) and goes radiatively stiff in a moist column (collapsing the adaptive dt under `sbm`). `exocol_io::read_initial_conditions` copies `cfg_msdist` into `exocol_mod::msdist` after the input file is read (file mode); `exocol_coldstart::cold_start_init` does the equivalent for cold-start mode.
...
- **`exocol_rce_loop`** — Time-marches the column with an adaptive CFL-limited timestep (`dt = cfl_safety · dT_target / max|HR|`, clamped to `[dt_min, dt_max] = [1e-4, 1.0]` days; `dT_target = 1 K`). Each step:
  ...
  2. bulk surface fluxes LE, SH (subcycled at `dt_sfc < τ_surf` for stability with thin bottom layers)
  ...
  5. `update_tint`
  6. **thermodynamic consistency loop** — `convadj` + `condense` (Newton satadj) iterated together up to `max_inner_phys = 20` times until `condense` removes no vapour (`precip_iter == 0`), then one final `convadj` pass. This ensures the column is simultaneously lapse-rate stable AND `q ≤ qsat` before each radiation step.
...
- **`exocol_convadj`** — Five schemes selectable via `conv_scheme`. The pairwise schemes (`dry`, `moist`, `manabe`, `zm`) operate purely on atmosphere-atmosphere pairs (no surface-bottom pair adjustment — surface coupling is handled by the bulk SH/LE fluxes) and conserve `cp·T·Δp` in each adjusted pair. The `'moist'` and `'manabe'` schemes also mix `h2ommr` in adjusted pairs. `sbm` is a whole-column scheme (see below).
