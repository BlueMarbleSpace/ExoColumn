...
The same local-copy pattern generates **three** n68equiv source files via `build/patch_exort.py` (the Makefile invokes it at parse time; it replaced the inline single-line `calc_opd` sed because the BPS toggle needs multi-line block insertions). Each patch is keyed to an explicit anchor string and the script exits non-zero (printing `ERROR`, no `PATCH_OK`) if any anchor disappears in a future ExoRT update; the Makefile treats the absence of the `PATCH_OK` sentinel as a hard `$(error ...)`. All three generated files (plus `exoplanet_mod.F90`, `sys_rootdir.F90`) are gitignored:
- **`src/calc_opd_mod.F90`** — (a) the k-coefficient pressure lookup is **clamped at the 10-bar table top** (`pressure = min(log10(pmid), log10pgrid(kc_npress))`). Above its pressure grid ExoRT linearly *extrapolates* k in log10(p), which is unphysical for >10 bar columns (outer-HZ dense-CO2 sweeps) and can produce negative k; the clamp freezes pressure broadening at its 10-bar value for deeper layers while CIA/continuum amagats keep the true `pmid`. Columns below 10 bar are bit-identical. (b) The MT_CKD H₂O self+foreign continuum block is wrapped in `if (.not. use_bps_continuum) … else … endif`; the `else` branch is the **BPS continuum** (see `h2o_continuum` below).
- **`src/radgrid.F90`** — adds the public runtime flag `use_bps_continuum` (default `.false.`) and the BPS filename. The BPS data arrays (`self`/`foreign`/`base_self`/`base_foreign`/`TempCoeff`) were already declared in stock n68equiv `radgrid` (dead declarations until now).
- **`src/initialize_rad_mod_1D.F90`** — conditionally reads `data/continuum/bps_h20_continuum_n68.nc` when `use_bps_continuum` is set.

**BPS continuum (`h2o_continuum`).** ExoRT's modern n68equiv tree applies MT_CKD 3.3 as an 8-point Gauss distribution; the legacy `src.n68h2o` tree carries the BPS (Wolf) continuum — the same continuum used by Kopparapu et al. (2013). `src.n68h2o` is **not** a usable drop-in (it is an older, H₂O-only model: HITRAN-2012, no CO₂/CH₄ line absorption, reduced CIA), so the BPS continuum path is **ported into n68equiv** as a runtime toggle rather than switching trees. `&exocol_nml::h2o_continuum = 'bps'` sets `radgrid::use_bps_continuum` (in `exocol_driver`, after `read_config`, before `initialize_kcoeff`), making the radiation core use the BPS layer-average self+foreign continuum across all 68 bands. Default `'mtckd'` keeps `use_bps_continuum=.false.` → **bit-identical** to all prior results. **Two caveats** when comparing to Kopparapu: (1) BPS here is a per-band layer average vs n68equiv's MT_CKD 8-gpt sub-band treatment; (2) it is paired with HITRAN-2016 lines + the n68equiv k-distribution, not CLIMA's line treatment — so it isolates the *continuum* difference but is a controlled comparison, not an identity.
...
exocol_driver (PROGRAM)
  ...
  └── exocol_sweep       :: run_flux_sweep (when sweep_mode; replaces the RCE loop)
...
- **`exocol_config`** — Reads `exocol_config.nml`. Three optional namelist blocks:
  - **`&exocol_nml`** (runtime physics): `conv_scheme` ∈ {`'dry'`, `'moist'`, `'manabe'`, `'zm'`, `'sbm'`} (default `'sbm'`, the recommended simplified Betts-Miller scheme), `moisture_scheme` ∈ {`'prognostic'`, `'fixed_rh'`, `'off'`}, `o3_profile` ∈ {`'uniform'`, `'earth'`, `'none'`, `'rcemip'`} (default `'uniform'`; see `exocol_ozone`), `wind_speed` (default 5 m/s), `C_D` (default 1.5e-3; legacy `'bulk'` exchange coefficient), `msdist` (planet-star distance in AU, default 1.0; TOA stellar flux scales as 1/msdist²), `rh_sbm` (default 0.7; SBM reference relative humidity), `surface_flux` ∈ {`'mos'`, `'bulk'`} (default `'mos'`; see `exocol_surface`), `z0_rough` (default 3.21e-5 m; surface roughness used by `'mos'`), `surface_water` (logical, default `.true.` → bit-identical; the wet/ocean surface evaporates at qsat(Ts) giving the latent heat flux LE. `.false.` = dry LAND surface — `exocol_surface::compute_surface_fluxes` forces `LE=0` via its optional `evap_on` argument (sensible heat SH unaffected). Required for a genuinely dry column, e.g. dry CO₂: with the default wet surface the slab loses LE = ρ·L·C·U·qsat(Ts) to "evaporation" that a `moisture_scheme='off'` column then discards — an energy sink that prevents radiative-convective equilibrium. Wired `exocol_config` → `exocol_rce_loop` `evap_on=surface_water`), `latent_heat_mode` ∈ {`'phase_aware'`, `'fixed_vap'`}, `esat_formula` ∈ {`'cc'`, `'steam'`} (default `'cc'`; `'steam'` = Wagner & Pruß 2002 saturation pressure accurate to the 647 K critical point, applied model-wide via the `exocol_convadj::esat` dispatcher), `h2o_eos` ∈ {`'ideal'`, `'nonideal'`} (default `'ideal'`; `'nonideal'` = Kasting 1988 Appendix-A general non-ideal moist pseudoadiabat for the cold-start temperature profile — see `exocol_steam`/`exocol_iapws95` — forces `esat_formula='steam'`, and below the 273.16 K triple point — where the IAPWS-95 liquid–vapour saturation does not exist — uses `exocol_convadj::twocomp_dlnTdlnP`, the two-component ideal-gas pseudoadiabat that carries the vapour heat capacity and saturates per `cold_trap_phase`; with the ice default this is term-for-term CLIMA's `convec.f` sub-freezing block, matching its T(P) to ±0.07 K. The previous textbook-`malr` fallback was ~13–18% too steep at runaway upper-troposphere mixing ratios — its virtual-T factor overshoots at ws ≳ 0.1 — freeze-drying the stratosphere ~2× at Ts = 360–380 K), `h2o_continuum` ∈ {`'mtckd'`, `'bps'`} (default `'mtckd'` → bit-identical; `'bps'` = Wolf BPS layer-average H₂O continuum ported from `src.n68h2o`, for direct Kopparapu et al. 2013 comparison — sets `radgrid::use_bps_continuum`; see the build section above for the two apples-to-apples caveats), `cold_trap_phase` ∈ {`'ice'`, `'liquid'`} (default `'ice'` → bit-identical; sets the saturation phase below the 273.16 K triple point via `exocol_convadj::set_cold_trap_phase`. `'ice'` saturates over ice/sublimation — physical for the cold trap **and** Kopparapu et al. (2013) CLIMA's actual convention (verified in the CLIMA source 2026-06-11: `SATRAT` uses CC with the sublimation latent heat below 273.16 K, and `convec.f` label 13 is the ice-saturated two-component pseudoadiabat = `twocomp_dlnTdlnP`). `'liquid'` saturates over supercooled liquid at all T — the WP curve extrapolated below the triple point in steam mode, or `esat_cc_liq` (CC with L_v) in cc mode — ~2× more cold-trap stratospheric H₂O at 200 K; retained as a sensitivity toggle (previously mislabeled "CLIMA convention"). The cold point is where it matters most; `esat_cc`/`malr` stay pure and unaffected), `coldtrap_dT_offset` (default 0 → bit-identical; when > 0 the cold-start stratospheric humidity is freeze-dried at the adiabat point where T = `t_strato` + offset instead of the exact `t_strato` crossing, and the constant-q region starts there — the T profile itself is unchanged. Purpose: emulate Kopparapu-2013 CLIMA's cold-trap *sampling*, whose tabulated profiles freeze-dry at the last vertical grid level below the 200 K cap, T* = 200.1–205.8 K depending on Ts, inflating their stratospheric H₂O ×1.0–2.1; used by `hz_inner.py`'s panel-(d) profile runs with their measured per-case T*−200. Radiatively negligible: < 0.7 W/m² OLR, < 0.003 Seff), `co2_vmr_total` (default `.false.` → bit-identical; when `.true.`, `co2_vmr` is interpreted as the mole fraction of **total moist air** at every layer — the Kopparapu/CLIMA convention (constant FCO₂, verified in their profiles) — so the cold-start CO₂ dry MMR is enhanced by 1/(1−f_H₂O) per layer. Negligible at the moist-GH end (−0.02 W/m² at Ts = 300 K) but NOT at the runaway end: a Tc-adjacent steam column holds ~200× more absolute CO₂ under this convention, lowering the steam-plateau OLR ~7 W/m² and the runaway Seff from 1.088 to 1.060. The default fixed-inventory convention (constant CO₂/N₂) is the physical choice; the toggle is for apples-to-apples sweeps), `co2_condense` and `cp_co2_tdep` (both default `.false.`; outer-HZ cold-start CO2 condensation + T-dependent cp_CO2 — see `exocol_co2`). `exocol_io::read_initial_conditions` copies `cfg_msdist` into `exocol_mod::msdist` after the input file is read (file mode); `exocol_coldstart::cold_start_init` does the equivalent for cold-start mode.
  ...
  - **`&exocol_sweep`** (flux-table sweep; see `exocol_sweep`): `sweep_mode` (default `.false.`; forces `flux_only`), `sweep_coszrs`, `sweep_albedo`, `sweep_ts`, `sweep_t_strato`, `sweep_outfile`. The four list variables hold up to `MAX_SWEEP = 64` entries each; unset slots carry the sentinel -1 and are ignored, so a sweep is specified just by listing the wanted values. An entry of `sweep_t_strato` below zero falls back to `&exocol_init::t_strato`; per-entry caps matter at the cold end, where a fixed 200 K stratosphere would sit *above* the surface temperature (`cold_start_init` then builds an isothermal 200 K column over, say, a 180 K surface). Leaving `sweep_ts` unset sweeps only zenith x albedo at `&exocol_init::ts`.
...
- **`exocol_sweep`** — Flux sweep over surface temperature x solar zenith angle x surface albedo, for building EBM radiation lookup tables. Activated by `&exocol_sweep::sweep_mode` (which forces `flux_only`); writes a plain-text record per combination to `sweep_outfile`. The point is to pay the ExoRT initialisation once: neither the zenith angle nor the surface albedo feeds back on the cold-start profile, so the inner (zenith x albedo) block reuses a single column at ~0.05 s per `aerad_driver` call against ~1.6 s of fixed init; the optional outer `sweep_ts` loop does rebuild the column (`cold_start_init`) but stays in the same process, so a whole temperature axis costs one initialisation. That matters more than the arithmetic suggests — a single-point run carries a ~240 MB working set, and repaying it per grid point makes a large table memory-bandwidth bound rather than compute bound. Verified bit-identical to one-process-per-temperature over 2200 records. Consumed by HEXTOR's `tools/make_radiation_table.py`. **Throughput note:** on the 6-core-plus-hyperthreading Xeon here the sweep saturates at ~38 records/s with 3-4 concurrent processes and *degrades* past that (12 concurrent measured 9 records/s, worse than one process) — parallelise by physical core, not logical CPU.
...
- **`exocol_convadj`** — Five convection schemes selectable via `conv_scheme`, plus two always-on helpers used with `'sbm'` (`convadj_surface`, the surface-coupled mixed layer; and `convadj_dry` reused as a non-deadlocking fallback). The pairwise schemes (`dry`, `moist`, `manabe`, `zm`) operate purely on atmosphere-atmosphere pairs and conserve `cp·T·Δp` in each adjusted pair. The `'moist'` and `'manabe'` schemes also mix `h2ommr` in adjusted pairs. `sbm` is a whole-column scheme (see below).
  ...
  - `'zm'`: Zhang-McFarlane soft adjustment (Zhang & McFarlane 1995, consistent with ExoCAM). For each unstable pair, the full moist-adiabatic correction is computed but only a fraction `f = 1 − exp(−dt/τ_conv)` is applied. With ExoColumn's default `τ_conv = 3600 s` (the CAM ZM closure uses 7200 s), each step removes ~75% of the instability near equilibrium (dt ≈ 0.06 d) vs ~100% during fast transients (dt >> τ_conv). This smooths discrete convective temperature jumps and reduces TOA flux noise by ~5–10×. Namelist parameters `tau_conv` [s] (default 3600) and `cape_trigger` [J/kg] (default 0, disabled) are set in `&exocol_nml`. The RCE loop applies one soft pass then condensation then one hard cleanup pass (f=1) per outer step — no inner iteration for the soft pass. `compute_cape` (public from `exocol_convadj`) returns surface-parcel CAPE [J/kg] and is used internally for the trigger check.
  ...
  `esat_cc(T)` is phase-aware (Clausius-Clapeyron with L_v above 0 °C, L_sub below; continuous at 273.16 K). Saturation vapour pressure is dispatched model-wide through `esat(T)` (set once at init via `set_esat_mode` from `&exocol_nml::esat_formula`): `'cc'` (default) returns `esat_cc`, `'steam'` returns the Wagner-Pruß `iapws95_psat_aux`. The non-pure `esat()` replaces `esat_cc` at every non-pure call site (condensation, the convective schemes, surface fluxes, cold-start); the **pure** functions `esat_cc` and `malr` are left untouched so the lapse-rate path stays pure and the default (`'cc'`) run is **bit-for-bit identical** to the historical behaviour (verified: the Earth RCE gives Ts=287.838… with and without these changes). `Lvap_T(T)` returns the phase-appropriate latent heat and is reused by `exocol_surface` and the condensation step. `compute_tint_interp(tmid, pint, nv, tint)` fills `tint(1:nv)` by log-pressure interpolation/extrapolation and is the single canonical implementation used by all three schemes and by `exocol_rce_loop::update_tint`.

- **`exocol_iapws95`** — Native Fortran port of the IAPWS-95 formulation for ordinary water (Wagner & Pruß 2002; IAPWS R6-95). Self-contained: carries the full canonical coefficient tables (ideal-gas `phi0` + the 56-term residual `phir` — polynomial, exponential, Gaussian, and the two non-analytic critical terms) with all δ/τ derivatives, the Table-3 property chain (`P`, `s`, `h`, `u`, `cv`, `cp`, speed of sound, isobaric expansivity, `Z`), a bracketed/safeguarded density solver `iapws95_PT(P,T,phase)`, a Maxwell-construction saturation solver `iapws95_sat(T)` (Wagner-Pruß 1992 auxiliary equations as initial guess), and a fast analytic `iapws95_psat_aux(T)`. No external data file. Validated point-by-point against the reference `iapws` Python package (single-phase props ~1e-13, saturation curve ~1e-9; `test/test_iapws95.F90` + `tools/check_iapws95.py`). The speed-of-sound `sqrt` is guarded so the EOS never raises a floating-invalid under `-fpe0`, even for out-of-range (e.g. sub-triple-point) states the solver may transiently probe.

- **`exocol_co2`** — CO2 condensation thermodynamics for the outer-HZ (maximum greenhouse) case: `psat_co2(T)` (Span & Wagner 1996 auxiliary equations, sublimation + liquid-vapour branches — the modern replacement for Kasting's Fanale et al. 1982 fits), `tsat_co2(p)` (bisection inverse / CO2 frost point), and `cp_co2(T)` (statistical-mechanics ideal-gas cp: 7/2 R + Einstein vibrational terms; matches NIST-JANAF to <0.5% over 150–650 K).  Self-contained (shr kinds/constants only).  Validated by `test/test_co2.F90` (triple point, 1-atm dry-ice point, critical point, JANAF cp anchors).  Consumed by `exocol_coldstart` under two `&exocol_nml` switches, **both default `.false.` (validated Earth/IHZ results bit-identical)**: `co2_condense` pins the standard cold-start adiabat to the CO2 saturation curve wherever it would supersaturate in CO2 (Kasting 1991; T(k) = max(T_adiabat, Tsat_CO2(vmr_co2·p), t_strato); CO2 stays well-mixed, CO2 clouds neglected — as in CLIMA), and `cp_co2_tdep` evaluates the CO2 share of cpdry at the local temperature inside the adiabat integration (Kopparapu 2013's Shomate update; `cpdry_col` itself stays constant).  NOTE: in `exocol_coldstart` the function must be imported renamed (`cp_co2_of_T => cp_co2`) because Fortran's case-insensitive names collide with the `CP_CO2` constant from `exocol_config`.  Reference case: `reference/max_greenhouse/` (Kopparapu 2013 §3.3 / Fig. 5 analogue; see its README for the diagnosed ExoRT-vs-CLIMA dense-CO2 radiation offsets and the >10-bar clamped-broadening caveat).  Result: maximum greenhouse Seff = 0.395 at pCO2 = 8.9 bar → d = 1.59 AU (Kopparapu: 0.337 at ~7 bar → 1.70 AU; both radiation offsets push our Seff high, so ours is the conservative bound).

- **`exocol_steam`** — Kasting (1988) Appendix-A general **non-ideal** moist pseudoadiabat, built on `exocol_iapws95`. `steam_dlnTdlnP_sat(T,P,Rd,cpdry)` assembles eq. A4 (`d ln αv/d ln T` from β, the saturated vapour/liquid entropies and their saturation-curve T-derivatives) then A5 (`d ln P/d ln T`) — carrying the heat capacity of **both** components, so it recovers the pure-steam saturated adiabat as `Pn→0` and the dilute textbook moist adiabat as `αv→0`. `steam_dlnTdlnP_dry(T,P,αv,Rd,cpdry)` is the unsaturated (constant-composition runaway-base) branch A11/A12 with the real-gas steam term `Cp/[P(∂V/∂T)p]` evaluated at the vapour partial state. Saturation-curve derivatives are central finite differences of `iapws95_sat(T±δT)` (the modern stand-in for Kasting's 5° steam-table differencing). Validated against an independent Python reimplementation (~1e-9) and against Kasting's ideal-gas analogs A7/A8 — agreement <0.3% where steam is near-ideal (≈300 K), diverging to tens of % approaching the critical point (`test/test_steam_adiabat.F90` + `tools/check_steam_adiabat.py`). Used by `exocol_coldstart` only when `h2o_eos='nonideal'`; below the 273.16 K triple point (no liquid-vapour saturation) the cold-start integration switches to `exocol_convadj::twocomp_dlnTdlnP` — the two-component **ideal-gas** pseudoadiabat (entropy budget with the Kirchhoff cancellation of the condensate heat capacity; β and the consistent effective L from finite differences of the `esat` dispatcher, vapour cp = `SHR_CONST_CPWV`). It matches `steam_dlnTdlnP_sat` to 0.03–0.45% across the 274–360 K overlap and, with the ice `cold_trap_phase` default, is term-for-term the sub-freezing moist adiabat of Kopparapu's CLIMA (`convec.f` label 13). (Historical: the textbook-`malr` fallback used before 2026-06-11 was ~13–18% too steep at high ws — virtual-T overshoot — drying the hot-case stratospheres ~2×.)
